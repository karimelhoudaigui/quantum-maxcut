...
export function GraphCanvas() {
  ...
  useEffect(() => {
    ...
    try {
      ...
      setRendererError("3D renderer unavailable in this browser context.");
      return undefined;
    }
      ...
      {rendererError ? (
        <div className="absolute inset-0 flex items-center justify-center bg-background/90 p-8 text-center">
          <div className="max-w-sm rounded-md border border-border bg-muted/45 p-5">
            <p className="text-sm font-semibold text-foreground">3D canvas unavailable</p>
            <p className="mt-2 text-sm text-foreground/60">{rendererError}</p>
          </div>
        </div>
      ) : null}
    </section>
